...
Also, in theory, new `Content` headers could be designed for granting permissions and authorizations to and between parts of complex MIME messages. This could resemble [permissions policies](https://w3c.github.io/webappsec-permissions-policy/) for `<iframe>` elements, for nested content, and by using HTTP headers.
