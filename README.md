...
Will artificial-intelligence APIs and agentic frameworks, one day, support expressiveness for "multipart" messages involving [`multipart/mixed`](https://en.wikipedia.org/wiki/MIME#mixed), [`multipart/related`](https://en.wikipedia.org/wiki/MIME#related) and [`multipart/alternative`](https://en.wikipedia.org/wiki/MIME#alternative)? Today, several artificial-intelligence message models include support for message content to be either a string or an array, to accept one or more content parts, e.g., to contain text accompanied by image data.
